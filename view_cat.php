...
<html lang="en">
  ...
  <body>
	...
	<?php
		...
		{
			$querycat = "AND brand='$cat' AND type_category='$type'";
			$catlink = "cat=$cat&";
		}else{
			if(!empty($type))
			{
				$querycat = "AND type_category='$type'";
			}else{
				$querycat = "";
			}
	...
	if ($temp[0] > 0){
		...
		// Рахуємо починаючи з якого номера
		// варто виводити товари
		$start = $page * $num - $num;
		...
		if (mysqli_num_rows($result) > 0){
			...
			echo '<div class="col-md-12" id="block-sorting">
			...
			do{
				...
				<div class="col-lg-4 col-md-6 mb-4">
				  <div class="card h-100">
					...
					<div class="card-footer">
					  ...
					  <div class="add-cart-block"><a href="" data-toggle="modal" data-target="#exampleModal'.$plus.'"><button id="add-cart"><img src="/images/addcart.png" width="24px" /></button></a></div>
					  <div class="review-block"><a href=""><button id="review">Перейти</button></a></div>
					</div>
				  </div>
				</div>
								...
								<div class="row">
									<div class="col-md-12">
										<div class="row text-center order-form">
											...
											<input type="text" name="post" placeholder="Відділення пошти *" required/>
											<input type="text" name="phone" placeholder="Номер телефону *" required/>
											<input class="submit_button" type="submit" value="Відправити" />
											</div>
										</div>
									</div>
								</div>
				...
				';
				include("include/modal-order2.php");
			}
			while($row = mysqli_fetch_array($result));
		}else{echo '<div class="col-12"><h1>Категорія недоступна або ще не створена</h1></div>';}
		
		if ($page != 1) {$pstr_prev = '<li><a class="pstr-prev" href="view_cat.php?page="'.($page - 1).'">&laquo;</a></li>';}
		if ($page != $total) $pstr_next = '<li><a class="pstr-next" href="view_cat.php?page='.($page + 1).'">&raquo;</a></li>';
		
		// Формуємо силки зі сторінками
		if($page - 5 > 0) $page5left = '<li><a href="view_cat.php?page='.($page - 5).'&type='.$type.'">'.($page - 5).'</a></li>';
		if($page - 4 > 0) $page4left = '<li><a href="view_cat.php?page='.($page - 4).'&type='.$type.'">'.($page - 4).'</a></li>';
		if($page - 3 > 0) $page3left = '<li><a href="view_cat.php?page='.($page - 3).'&type='.$type.'">'.($page - 3).'</a></li>';
		if($page - 2 > 0) $page2left = '<li><a href="view_cat.php?page='.($page - 2).'&type='.$type.'">'.($page - 2).'</a></li>';
		if($page - 1 > 0) $page1left = '<li><a href="view_cat.php?page='.($page - 1).'&type='.$type.'">'.($page - 1).'</a></li>';
		
		if($page + 5 <= $total) $page5right = '<li><a href="view_cat.php?page='.($page + 5).'&type='.$type.'">'.($page + 5).'</a></li>';
		if($page + 4 <= $total) $page4right = '<li><a href="view_cat.php?page='.($page + 4).'&type='.$type.'">'.($page + 4).'</a></li>';
		if($page + 3 <= $total) $page3right = '<li><a href="view_cat.php?page='.($page + 3).'&type='.$type.'">'.($page + 3).'</a></li>';
		if($page + 2 <= $total) $page2right = '<li><a href="view_cat.php?page='.($page + 2).'&type='.$type.'">'.($page + 2).'</a></li>';
		if($page + 1 <= $total) $page1right = '<li><a href="view_cat.php?page='.($page + 1).'&type='.$type.'">'.($page + 1).'</a></li>';
